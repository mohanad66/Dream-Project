...
export const fetchCarouselImages = () =>
  cacheGet("carousel_images", "/api/carousels/");
  ...
  const fetchAllData = useCallback(
    async (isAuth = false) => {
      try {
        ...
        if (ENABLE_CACHE) {
          const cachedData = await persistentCache.get(
            cacheKey,
            DATA_CACHE_TTL,
          );
          if (cachedData) {
        ...
        if (isAuth) {
          ...
          sellerPromise = api.get("/api/sellers/me/").catch(() => {
            return { data: null }; // Not a seller or error
          });
        }

        const results = await Promise.all([
          ...requests,
          ...(userPromise ? [userPromise, sellerPromise] : []),
        ]);
        ...
        const freshData = {
          ...
          sellerProfile: isAuth ? results[7]?.data : null,
        };
        ...
        setAuthData({
          ...
          isSeller: !!freshData.sellerProfile,
        });
        return true;
      } catch (err) {
...
export const fetchAllCategories = (url) =>
  fetchPaginatedData(url, `all-categories-${url}`);

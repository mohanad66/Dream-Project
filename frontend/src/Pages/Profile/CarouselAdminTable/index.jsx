...
const CarouselSlideCard = ({
  ...
  return (
    ...
    >
      ...
      <span
        className={`carousel-slide-card__status ${item.is_active ? "carousel-slide-card__status--on" : "carousel-slide-card__status--off"}`}
      >
        {item.is_active ? "● Live" : "○ Hidden"}
      </span>

      <div className="carousel-slide-card__content">
        <h5 className="carousel-slide-card__title">
          {item.title || "Untitled Slide"}
        </h5>
        {item.subtitle && (
          <p className="carousel-slide-card__sub">{item.subtitle}</p>
        )}
        {item.link && (
      ...
      <div
        className={`carousel-slide-card__actions ${hovered || isPending ? "carousel-slide-card__actions--visible" : ""}`}
      >
        <button
...
const CarouselEmptyState = ({ onAdd }) => (
  <div className="carousel-empty">
    ...
    <p className="carousel-empty__text">
      No slides yet — start building your carousel
    </p>
        ...
        {active.map((item, i) => (
          <div key={item.id} className="carousel-preview-strip__thumb">
            {item.image ? (
              ...
              <div className="carousel-preview-strip__placeholder" />
            )}
            <span>{i + 1}</span>
          </div>
        ))}
    ...
    <div className="carousel-management-section management-section">
      <div className="carousel-mgmt-header">
        <div className="carousel-mgmt-header__left">
          ...
          <div>
            <h4 className="carousel-mgmt-header__title">
              Carousel Slides
              <span className="carousel-mgmt-header__count">
                {carouselPagination.count}
              </span>
            </h4>
            <p className="carousel-mgmt-header__sub">
              {carouselItems.filter((i) => i.is_active).length} live ·{" "}
              {carouselItems.filter((i) => !i.is_active).length} hidden
            </p>
          </div>
        </div>
        <div className="carousel-mgmt-header__right">
          ...
          <button
            className="button button--primary button--small carousel-add-btn"
            onClick={() => onOpenCarouselModal(null)}
          >
            <svg
              width="14"
              height="14"
      ...
      {carouselLoading ? (
        <div className="carousel-skeleton-grid">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="carousel-skeleton-card"
              style={{ "--i": i }}
            >
              <div className="skeleton-shimmer" />
            </div>
      ...
      {carouselItems.length > 0 && (
        <div className="pagination-controls">
          <button
            onClick={() =>
              onRefreshCarousel(carouselPagination.currentPage - 1)
            }
            disabled={!carouselPagination.previous || carouselLoading}
            className="button"
          >
            Previous
          </button>
          <span>
            Page {carouselPagination.currentPage} of {totalPages} (
            {carouselPagination.count} total)
          </span>
          <button
            onClick={() =>
              onRefreshCarousel(carouselPagination.currentPage + 1)
            }
            disabled={!carouselPagination.next || carouselLoading}
            className="button"
          >
            Next
          </button>

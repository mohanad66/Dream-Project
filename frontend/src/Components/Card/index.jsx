      ...
      <div className="card">
        ...
        {card.effective_price && parseFloat(card.effective_price) < parseFloat(card.price) ? (
          <span className="price" style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
            <span style={{ color: "var(--color-success, #3fa781)", fontWeight: 700 }}>{parseFloat(card.effective_price).toFixed(2)} L.E</span>
            <span style={{ textDecoration: "line-through", color: "var(--text-muted)", fontSize: "0.85em" }}>{parseFloat(card.price).toFixed(2)} L.E</span>
          </span>
        ) : (
          <span className="price">{parseFloat(card.price || 0).toFixed(2)} L.E</span>
        )}
                    ...
                    <p className="card-price">
                      {card.effective_price && parseFloat(card.effective_price) < parseFloat(card.price) ? (
                        <span style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
                          <span style={{ color: "var(--color-success, #3fa781)", fontWeight: 700 }}>Price: {parseFloat(card.effective_price).toFixed(2)} L.E</span>
                          <span style={{ textDecoration: "line-through", color: "var(--text-muted)", fontSize: "0.85em" }}>{parseFloat(card.price).toFixed(2)} L.E</span>
                        </span>
                      ) : (
                        `Price: ${parseFloat(card.price || 0).toFixed(2)} L.E`
                      )}
                    </p>
                  )}
                  {card.category ? (
